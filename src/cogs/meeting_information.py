...
class meeting_cogs(commands.Cog):
    ...
    async def new_meeting(self, ctx,
        ...
        emb.add_field(name="Meeting ID:", value=meeting_id, inline=False)
        emb.add_field(name="Meeting Name:", value=name, inline=False)
        emb.add_field(name="Time:", value=time_obj.strftime("%m/%d/%Y %H:%M:%S"), inline=False)
